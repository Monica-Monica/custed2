  ...
  export default {
    ...
    methods: {
      handleSizeChange(val) {
        ...
        this.getNewsList(this.listQuery.page, this.listQuery.limit, this.newsType, this.bindQuery.id)
      },
      ...
      getNewsList(page, limit, type, bindId) {
        if (this.bindQuery.id) {
          ...
          this.$message.error('请重新选择一个指数查看')
          this.$router.push({ name: 'IndexsManagement' })
        }
      },
      // 获取资讯详情
      getNewsDetail(id) {
    ...
    created() {
      this.getNewsList(1, 10, this.newsType, this.bindQuery.id)
    }
  }
</script>
