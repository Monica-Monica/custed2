...
<script>
  ...
  export default {
    ...
    computed: {
      bindQuery() {
        return this.$store.getters.bindQuery
      }
    },
    methods: {
      handleSizeChange(val) {
        this.listQuery.limit = val
        this.getNewsList(this.listQuery.page, this.listQuery.limit, this.newsType, this.bindQuery.coinId)
      },
      ...
      getNewsList(page, limit, type, bindId) {
        if (this.bindQuery.coinId) {
          this.listLoading = true
          newsList(page, limit, type, bindId).then(res => {
            console.log(res)
            if (res.data.code === 0) {
          ...
          this.$message.error('请重新选择一个货币查看')
          this.$router.push({ name: 'CurrencyManagement' })
        }
      },
      // 获取资讯详情
      getNewsDetail(id) {
    ...
    created() {
      this.getNewsList(1, 10, this.newsType, this.bindQuery.coinId)
    }
  }
</script>
