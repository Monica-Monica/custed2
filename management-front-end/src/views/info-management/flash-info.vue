<template>
  <div class="app-container flash-info-management-container">
    ...
    <div class="filter-container">
      ...
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate">创建快讯</el-button>
    </div>

    <el-table :key='tableKey' :data="flashNewsLists" v-loading.body="listLoading" element-loading-text="Loading" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="操作" class-name="small-padding fixed-width" width="300">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" plain @click="showFlashDetail(scope.row)">详情</el-button>
    ...
    <el-dialog :title="dialogTitle[dialogType] + '快讯'" :visible.sync="updateDialogVisible">
      <el-form :model="updateForm">
        <el-form-item label="标题" :label-width="formLabelWidth">
          <el-input v-model="updateForm.title" auto-complete="on"></el-input>
        </el-form-item>
      </el-form>
...
<script>
  import { newsList, newsDetail, updateNews, deleteNews, createNews } from '../../api/api'

  export default {
    name: 'flash-info-management',
    data() {
      return {
        flashNewsLists: [],
        newsType: 2,
    ...
    methods: {
      ...
      // 创建新资讯
      handleCreate() {
        this.updateForm = { title: '', content: '', type: '' }
      ...
      handleCommit(data) {
        if (this.dialogType === 'showUpdate') {
          this.updateNewsRequest(data)
        } else if (this.dialogType === 'createUpdate') {
          this.createNewsRequest(data)
        }
      },
      // 删除快讯
      handleDelete(row) {
      ...
      getNewsList(page, limit, type) {
        this.listLoading = true
        newsList(page, limit, type).then(res => {
          console.log(res)
          if (res.data.code === 0) {
            this.flashNewsLists = res.data.data.list
      ...
      createNewsRequest(data) {
        createNews(data.title, data.content, this.newsType, '0', '').then(res => {
          console.log(res)
          if (res.data.code === 0) {
            this.getNewsList(this.listQuery.page, this.listQuery.limit, this.newsType)
            this.$message({
...
<style rel="stylesheet/scss" lang="scss">
  .flash-info-management-container{
    .image {
      width: 100%;
      display: block;
    }
